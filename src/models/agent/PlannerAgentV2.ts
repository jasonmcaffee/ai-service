...
class PlannerAgentFunctionContext implements AiFunctionContextV2 {
  ...
  public functionResultsStorage = {};//where we house "$aiAdd.result" etc
  constructor(
...
export default class PlannerAgentV2 implements AiFunctionExecutor<PlannerAgentV2> {
  ...
  async askAiToCreateAnAgentPlan(userPrompt: string){
    ...
    const topicId = uuid();
    this.inferenceSSESubject?.sendStatus({topicId, topic: 'planning', displayText: `Creating a plan to determine which tools to use.`, });
